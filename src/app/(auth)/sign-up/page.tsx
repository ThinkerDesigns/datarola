...
function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Don't redirect here — let the useEffect above handle it, but only after loading clears
      // If we're already on sign-up and signup just succeeded, show success state instead
    } catch {
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#0a0e1a] px-4">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-slate-500">Start asking questions about your data.</p>
        </div>
        ...
        <p className="text-center text-xs text-slate-500">
          Already have an account?{' '}<a href="/sign-in" className="text-brand-400 hover:text-brand-300">Sign in</a>
        </p>
      </div>
    </div>
  );
